...
int cp_sokdl_sig(bn_t c, bn_t s, uint8_t *msg, size_t len, ec_t y, bn_t x) {
	bn_t n, r;
	ec_t t;
...
int cp_sokdl_ver(bn_t c, bn_t s, uint8_t *msg, size_t len, ec_t y) {
	bn_t n, v;
	ec_t t;
	uint8_t h[RLC_MD_LEN];
	uint8_t *buf, *m = RLC_ALLOCA(uint8_t, len + 3 * (RLC_FC_BYTES + 1));
...
int cp_sokor_sig(bn_t c[2], bn_t s[2], uint8_t *msg, size_t len, ec_t y[2],
		bn_t x, int first) {
	bn_t n, v[2], z;
	ec_t g, t[2];
	uint8_t h[RLC_MD_LEN];
...
int cp_sokor_ver(bn_t c[2], bn_t s[2], uint8_t *msg, size_t len, ec_t y[2]) {
	bn_t n, v[2], z;
	ec_t t;
	uint8_t h[RLC_MD_LEN];
	uint8_t *buf, *m = RLC_ALLOCA(uint8_t, len + 6 * (RLC_FC_BYTES + 1));
